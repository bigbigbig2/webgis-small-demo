...
//版权信息
var mapAttr = 'Map data &copy; <a href="https://www.openstreetmap.org/">哈哈与黑洞大笑</a> contributors, ' +
'<a href="http://giscafer.com/">giscafer</a>, ' +
'Imagery © <a href="https://www.mapbox.com/">Mapbox</a>';

// 定义两个图层，影像图层和街道图层（通过mapbox地图服务获取）
var satellite = L.tileLayer(mapboxUrl, { 
    id: 'mapbox.satellite', attribution: mapAttr 
});
var streets = L.tileLayer(mapboxUrl, {
    id: 'mapbox.streets', attribution: mapAttr 
    });
...
//  数据转换
var allpoints = dataHandler();  //获取数据
    ...
    var timer = setInterval(function () {
        if (count < length) {
            ...
            if (count === length) {
                ...
                marker = L.marker(drawPoints[length - 1], { icon: typhoonIcon }).addTo(map)
                    .bindPopup("<b>" + typhoonTestData[0]['name'] +
                        "</b><br>" + land['info'] + "<br>" + land['landtime'] + "<br>经度：" + land['lng'] + "<br>纬度：" + land['lat'] + "<br>强度：" + land['strong'] + "<br><br>")
                    .openPopup();
            } else {
                marker = L.marker(drawPoints[count - 1], { icon: typhoonIcon }).addTo(map)
            }
